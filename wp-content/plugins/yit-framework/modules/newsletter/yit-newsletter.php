...
class YIT_Newsletter{
    ...
    public function __construct() {
        ...
        $sc_lists = $this->newsletter_shortcode_list();

        foreach( $sc_lists as $shortcode => $atts ){
    ...
    public function init_newsletter_popup_frontend(){
        ...
        if ( function_exists( 'WC' ) && ( is_shop() || is_product_category() || is_product_tag() ) ) {
            if ( function_exists( 'wc_get_page_id' ) ) {
                ...
                $post_id = (version_compare( WC()->version, '3.0.0', '<' ) ) ? woocommerce_get_page_id( 'shop' ) : wc_get_page_id( 'shop' );;
            }
        }
